#!/usr/bin/env python3
"""
VideoReel Generator - FIXED VERSION WITH ROTATION
Extracts interesting segments from B-roll footage and preserves rotation
"""
...
def get_rotation(container) -> int:
    ...
    for stream in container.streams.video:
        # Get first frame to check side data
        for frame in container.decode(video=0):
            if frame.side_data:
                for side_data in frame.side_data:
                    # Check if it's display matrix (contains rotation)
                    if 'DISPLAYMATRIX' in str(side_data.type):
                        # Display matrix indicates rotation
                        # For phone videos, typically 90° clockwise
                        # This is a simplification - proper parsing would read the matrix
                        print(f"  Found rotation metadata: {side_data}")
                        return 90  # Common for portrait phone videos
            break  # Only check first frame
        break  # Only check first stream
    return 0
...
def rotate_frame(frame_array: np.ndarray, rotation: int) -> np.ndarray:
    ...
    Args:
        frame_array: NumPy array of frame (H x W x 3)
    ...
    if rotation == 0:
        ...
        # Rotate 90 counter-clockwise to correct portrait phone videos
...
class ReelExtractor:
    ...
    def extract_segments(self, video_path: str, segments: List[Dict],
        ...
        """Extract video segments with rotation correction"""
    ...
    def _extract_segment(self, input_path: str, start_time: float,
                        end_time: float, output_path: Path, rotation: int = 0):
        """Extract a single segment with rotation correction"""
        ...
        try:
            ...
            if rotation in [90, 270]:
                output_width = input_stream.height
                output_height = input_stream.width
            else:
                output_width = input_stream.width
                output_height = input_stream.height
            ...
            # Track first PTS to calculate offset (proper timestamp reset)
            first_video_pts = None
            first_audio_pts = None

            # Encode frames with rotation
            for packet in input_container.demux(video=0, audio=0 if audio_stream else None):
                if packet.stream.type == 'video':
                    for frame in packet.decode():
                        ...
                        # Store original PTS before rotation
                        ...
                        # Proper PTS reset: offset by first frame's PTS
                        if first_video_pts is None:
                            first_video_pts = original_pts

                        frame.pts = original_pts - first_video_pts
                    ...
                    for frame in packet.decode():
                        ...
                        # Proper PTS reset for audio too
...
def main():
    parser = argparse.ArgumentParser(
        description='Extract interesting reels from B-roll footage (WITH ROTATION FIX)'
    )
    parser.add_argument('input', nargs='+', help='Input video file(s)')
    parser.add_argument('-o', '--output', default='reels_fixed',
                       help='Output directory (default: reels_fixed/)')
